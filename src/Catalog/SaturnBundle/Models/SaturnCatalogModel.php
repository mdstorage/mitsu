...
namespace Catalog\SaturnBundle\Models;


use Catalog\CommonBundle\Components\Constants;
use Catalog\CommonBundle\Models\CatalogModel;
use Catalog\SaturnBundle\Components\SaturnConstants;

class SaturnCatalogModel extends CatalogModel{
    ...
    public function getModels($regionCode)
    {
        ...
        FROM model
        WHERE MAKE_DESC = 'Saturn'
        AND (COUNTRY_CODE = '*' OR COUNTRY_CODE = :regionCode)

        UNION
        SELECT MODEL_DESC, CATALOG_CODE, MODEL_ID, MAKE_DESC
        FROM model
        WHERE MAKE_DESC = 'Lt Truck Saturn'
        AND (COUNTRY_CODE = '*' OR COUNTRY_CODE = :regionCode)
        ORDER BY (1)
        ";
