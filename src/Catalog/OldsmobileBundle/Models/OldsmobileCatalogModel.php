...
namespace Catalog\OldsmobileBundle\Models;


use Catalog\CommonBundle\Components\Constants;
use Catalog\CommonBundle\Models\CatalogModel;
use Catalog\OldsmobileBundle\Components\OldsmobileConstants;

class OldsmobileCatalogModel extends CatalogModel{
    ...
    public function getModels($regionCode)
    {
        ...
        FROM model
        WHERE MAKE_DESC = 'Oldsmobile'
        AND (COUNTRY_CODE = '*' OR COUNTRY_CODE = :regionCode)

        UNION
        SELECT MODEL_DESC, CATALOG_CODE, MODEL_ID, MAKE_DESC
        FROM model
        WHERE MAKE_DESC = 'Lt Truck Oldsmobile'
        AND (COUNTRY_CODE = '*' OR COUNTRY_CODE = :regionCode)
        ORDER BY (1)
        ";
